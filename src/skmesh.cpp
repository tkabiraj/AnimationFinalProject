...
void SkinnedMesh::ReadNodeHeirarchy(const aiNode* pNode, const glm::mat4& ParentTransform, double AnimationTime, double AnimationTime_to, double blendfactor)
    {
...
//            glm::mat4 NodeTransformation2 = mat4_cast(pNode->mTransformation);

            const aiNodeAnim* pNodeAnim2 = FindNodeAnim(pAnimation2, NodeName);
            vec3 scale2,translation2; quat rotation2;
            GetNodeTransform(pNodeAnim2, AnimationTime,scale2, rotation2, translation2);
            
            vec3 scaleFactor = (1.0f - (float)blendfactor)*scale + scale2*(float)blendfactor;
            vec3 translateFactor = (1.0f - (float)blendfactor)*translation + translation2*(float)blendfactor;
            quat rotateFactor = slerp(rotation, rotation2, (float)blendfactor);
            
            ScalingM = glm::scale(glm::mat4(1.0f), scaleFactor);
            TranslationM = glm::translate(glm::mat4(1.0f), translateFactor);
            RotationM = mat4(rotateFactor);
        }
    ...
    if (nextAnimation >= 0)
            ...
            if (AnimationTime < time_blend_start_from) //already running, finished?
                ...
                double deltatime_period_actual_to = animduration_to - time_blend_start_to;
                    blendfactor = (AnimationTime - time_blend_start_from)/(animDuration - time_blend_start_from);
